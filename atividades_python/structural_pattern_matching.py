...
def execute_command(command):
    match command.split():
        # Caso o primeiro item da lista gerado pelo split seja 'ls',
        # o segundo seja path - path se torna uma variável e pode ter qualquer coisa - atribui a variável o valor
        # e o terceiro pode ser qualquer coisa (*_) - quando coloca * é uma 'lista', pode percorrer a variavel
        case ['ls', path, *_]:
            print('$ listing files from', path)
        case ['cd', path]:
            print('$ changing directory to', path)
        case _:  # Não obrigatório - else
            print('$ command not implemented')
...
def execute_command(command):
    match command.split():
        # O Pipe '|' funciona como operador OR para o case
        case ['ls' | 'list', path, *_]:
            print('$ listing files from', path)
        case ['cd', path]:
            print('$ changing directory to', path)
...
def execute_command(command):
    match command.split():
        # Pode ser inserido um if depois do case que será executado
        # Só cai na condição se o if também for verdadeiro
...
def execute_command(command):
    match command.split():
        # Usando com 'as' atribui o valor do comando por ex para uma variavel
...
def execute_command(command):
    match command:
        # Também da match com dicionários
        # Command precisa ser 'ls'
        # Precisa ter a chave directories com 1 ou mais elementos (n pode ser vazia)
