'''
    Calculadora desenvolvida utilizando PyQt5
'''
...
class Calculadora(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle('Calculadora do Danton')
        ...
        self.display.setDisabled(True) # Desabilita para nao permitir digitar
        self.display.setStyleSheet(
            '* {background: white; color: #000; font-size: 30px;}'
        ) # Seta por meio de CSS o estilo
        self.display.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding) # Se expande conforme o espaço da janela
    ...
    def add_btn(self, btn, row, col, rowspan, colspan, funcao=None, style=None):
        ...
        if not funcao:
            btn.clicked.connect(
                ...
                ) if self.display.text() != 'Conta inválida.'
                else self.display.setText(btn.text())
            )
        else:
